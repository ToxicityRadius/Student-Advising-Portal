...
const PlanOfStudy = () => {
  ...
  const semesterCards = useMemo(() => {
    ...
    return semesters
      .map((semester, index) => {
        const allCourses = (semester.courses || []).map((course) => ({
          ...course,
        ...
        const units = remainingCourses.reduce((sum, course) => sum + Number(course.units || 0), 0);
  ...
  const canExportValidatedPlan =
    reviewPresentation.variant === 'approved' && Boolean(dashboardData?.sarId);
  ...
  const handleExportValidatedPlan = async () => {
    ...
    try {
      ...
      setExportError(
        error?.response?.data?.message || 'Unable to export Study Plan PDF right now.',
      );
    } finally {
  ...
  return (
    <StudentLayout activePage="plan-of-study" pageTitle="Study Plan">
      <div className="pos-page">
        ...
        {loading ? (
          <section className="pos-panel pos-loading">Loading study plan...</section>
        ) : error ? (
          ...
          <>
            <section className="pos-summary-card">
              ...
              <div
                className={`pos-summary-review pos-summary-review--${reviewPresentation.variant}`}
              >
              ...
              {exportError && <div className="pos-summary-export-error">{exportError}</div>}
              ...
              {(semesterCards || []).map((semester, idx) => {
                ...
                return (
                  <article key={semester.key} className="pos-semester-card">
                    <div className="pos-dot" style={{ top: idx === 0 ? 14 : 26 }} />
                    <button
                      type="button"
                    ...
                    {isOpen && (
                      <div className="pos-courses">
                        {(semester.courses || []).map((course, i) => (
                          ...
                          >
                            <span className="pos-course-code">{course.code || 'CPE 000'}</span>
                            <span className="pos-course-name">
                              {course.name || 'Course title unavailable'}
                            </span>
                            <span className="pos-course-units">{course.units || 0} units</span>
                            <span
