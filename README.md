...
- **Platform-Wide Pagination Standardization (Phase 7):** List-heavy backend APIs now share a unified pagination contract (`page`, `pageSize`, `search`, `sortBy`, `sortOrder` → `items` + `meta`), and key frontend pages now use consistent server-side pagination controls (10/12/15 per page). This includes adviser student records, curriculum management tabs, term management, transfer ownership, and forecast history, with verified search/sort/page interactions on multi-page datasets.
