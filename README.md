...
- **SAR Academic Intelligence Engine (Phase 8):** SAR detail responses now include a structured `analytics` payload generated by a centralized backend service, covering completion KPIs (units completed/remaining, completion %), subject status indicators and counters, prerequisite eligibility with explicit unmet prerequisites, GPA/GWA monitoring, semester summaries, adviser review workflow status, priority subjects, remaining semester tracking, and estimated graduation output. These analytics are now consumed across student/adviser SAR pages, student dashboard summaries, and SAR PDF export.
