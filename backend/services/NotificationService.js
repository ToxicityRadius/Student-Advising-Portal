...
const NOTIFICATION_TEMPLATES = {
  study_plan_validated: {
    ...
    body: (meta) => `Your study plan (v${meta.versionNumber || '?'}) has been validated by your adviser.`
  },
  study_plan_regenerated: {
    type: 'info',
    title: 'New study plan draft available',
    body: (meta) => `A new draft study plan (v${meta.versionNumber || '?'}) has been generated based on your latest grades.`
  },
  grades_entered: {
    type: 'info',
  ...
  elective_track_selected: {
    ...
    body: (meta) => `Elective track "${meta.trackName || ''}" has been set for your academic record.`
  }
};
 ...
 */
async function notify({ recipientId, actorId, category, resourceType, resourceId, meta = {}, title, body, type }) {
 ...
 */
async function notifyMany({ recipientIds, actorId, category, resourceType, resourceId, meta = {}, title, body, type }) {
  const results = [];
  for (const recipientId of recipientIds) {
    const result = await notify({ recipientId, actorId, category, resourceType, resourceId, meta, title, body, type });
...
async function getNotifications(userId, { page = 1, pageSize = 50, unreadOnly = false } = {}) {
  ...
  const { rows, count } = await Notification.findAndCountAll({
    ...
    include: [
      { model: User, as: 'Actor', attributes: ['id', 'firstName', 'lastName', 'role'], required: false }
    ]
  });
