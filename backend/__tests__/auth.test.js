...
jest.mock('../models', () => {
  ...
  return {
    ...
    StudentAcademicRecord: { findAll: jest.fn().mockResolvedValue([]), findOne: jest.fn().mockResolvedValue(null), update: jest.fn().mockResolvedValue([0]) },
...
describe('Auth API', () => {
  ...
  describe('POST /api/auth/login', () => {
    ...
    test('returns 401 for wrong password', async () => {
      ...
      const user = mockUserInstance({
        ...
        role: 'student', isActive: true, failedLoginAttempts: 0,
        lockedUntil: null, mustChangePassword: false, mustChangeEmail: false
      });
    ...
    test('returns 429 when account is locked', async () => {
      ...
      const user = mockUserInstance({
        id: 1, email: 'locked@test.com', password: hashed,
        role: 'student', isActive: true, failedLoginAttempts: 5,
        lockedUntil: Date.now() + 15 * 60 * 1000,
        mustChangePassword: false, mustChangeEmail: false
      });
      User.findOne.mockResolvedValue(user);
    ...
    test('returns token on successful login', async () => {
      const hashed = await bcrypt.hash('Password1!', 10);
      const user = mockUserInstance({
        id: 2, email: 'student@test.com', password: hashed,
        role: 'student', isActive: true, failedLoginAttempts: 0,
        lockedUntil: null, mustChangePassword: false, mustChangeEmail: false,
        firstName: 'Test', lastName: 'User', studentId: '1234567',
        is_verified: true
      });
      User.findOne.mockResolvedValue(user);
    ...
    test('returns 401 when account is inactive', async () => {
      const hashed = await bcrypt.hash('Password1!', 10);
      const user = mockUserInstance({
        id: 3, email: 'inactive@test.com', password: hashed,
        role: 'student', isActive: false, failedLoginAttempts: 0,
        lockedUntil: null, mustChangePassword: false, mustChangeEmail: false
      });
      User.findOne.mockResolvedValue(user);
  ...
  describe('GET /api/auth/me', () => {
    ...
    test('returns user data with valid token', async () => {
      const user = mockUserInstance({
        id: 5, email: 'me@test.com', role: 'student',
        firstName: 'Jane', lastName: 'Doe', isActive: true,
        password: 'hashed', sex: 'Female',
        mustChangePassword: false, mustChangeEmail: false
      });
      User.findByPk.mockResolvedValue(user);
    ...
    test('returns 401 when user is deactivated', async () => {
      const user = mockUserInstance({
        id: 6, email: 'deactivated@test.com', role: 'student',
        isActive: false
      });
      User.findByPk.mockResolvedValue(user);
  ...
  describe('Role-based access control', () => {
    test('admin can access admin-only routes', async () => {
      const admin = mockUserInstance({
        id: 10, email: 'admin@test.com', role: 'admin',
        isActive: true, mustChangePassword: false, mustChangeEmail: false
      });
    ...
    test('student cannot access admin-only user listing', async () => {
      const student = mockUserInstance({
        id: 11, email: 'student@test.com', role: 'student',
        isActive: true, mustChangePassword: false, mustChangeEmail: false
      });
